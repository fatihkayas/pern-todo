...
function App() {
  ...
  const addToCart = (watch) => setCart([...cart, watch]);
                ...
                <>
                  {cart.map((item, index) => (
                    <div key={index} className="d-flex justify-content-between align-items-center mb-2 p-2 border-bottom">
                      ...
                      <button className="btn btn-sm btn-outline-danger" onClick={() => removeFromCart(index)}>Sil</button>
                    </div>
                  ))}
                  <button className="btn btn-primary w-100 mt-3" onClick={() => alert("Sipariş alındı!")}>Ödemeye Geç</button>
                </>
              )}
            </div>
          </div>
