    ...
    <>
      ...
      >
        <div className="d-flex justify-content-between align-items-center p-3 border-bottom">
          <h5 className="mb-0">{IS_PIZZA ? "Cart" : "Cart"} ({totalCount})</h5>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "1rem" }}>
          {!IS_PIZZA ? (
            ...
            <>
              {pizzaCart.length === 0 ? <p className="text-muted text-center mt-4">Your cart is empty</p> : null}
              {pizzaCart.map((item) => (
                <div key={item.cart_item_id} className="d-flex align-items-center gap-2 mb-3 p-2 border rounded">
                  <img
                    src={item.image_url || "https://images.unsplash.com/photo-1574071318508-1cdbab80d002?w=60"}
                    alt={item.name}
                    style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 8 }}
                  />
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#e67e22" }}>
                      {item.options.side ? `Side: ${item.options.side} · ` : ""}
                      {item.options.sauces?.length ? `Sauces: ${item.options.sauces.join(", ")} · ` : ""}
                  ...
                  <div>
                    ...
                    <button className="btn btn-sm btn-link text-danger p-0 mt-1" onClick={() => removePizzaFromCart?.(item.cart_item_id)}>Remove</button>
                  </div>
                </div>
              ))}
            </>
