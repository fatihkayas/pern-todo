...
const Register = ({ onLogin }: RegisterProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Welcome, ${data.customer.full_name}!`);
      navigate("/");
    } catch (err) {
      clearAuthData();
  ...
  return (
    <div className="container" style={{ maxWidth: 480, marginTop: 60, marginBottom: 60 }}>
      <div className="card shadow-sm border-0 rounded-4 p-4">
        ...
        <p className="text-center text-muted small mb-4">Set up your customer profile</p>

        <div className="d-flex justify-content-center gap-2 mb-4">
          <span className={`badge rounded-pill ${step === 1 ? "bg-dark" : "bg-secondary"}`}>
            1. Account
          </span>
          <span className={`badge rounded-pill ${step === 2 ? "bg-dark" : "bg-secondary"}`}>
            2. Address
          </span>
        </div>
